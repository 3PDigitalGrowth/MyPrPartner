...
export default function Personas({ content }: { content: PersonasContent }) {
  ...
  return (
    <section className="mt-14 animate-fade-in-up md:mt-16">
      <SectionEyebrow>
        <MT id={cid("personas.eyebrow")}>{content.eyebrow}</MT>
      </SectionEyebrow>
      <SectionHeading>
        <MT id={cid("personas.heading")}>{content.heading}</MT>
      </SectionHeading>
      <p className="mt-4 text-[16px] leading-relaxed text-text-medium">
        <MT id={cid("personas.intro")}>{content.intro}</MT>
      </p>

      <div className="mt-7 grid grid-cols-1 gap-5 sm:grid-cols-2">
        {content.items.map((p, i) => {
          const imgId = cid(`personas.items.${i}.image`);
          return (
            <div
              key={p.title}
            ...
            >
              <div className="relative aspect-[16/9] overflow-hidden">
                <Image
                  src={imgId ? copySrc(imgId, p.image) : p.image}
                  alt=""
                  fill
                  sizes="(min-width: 1024px) 40vw, (min-width: 640px) 45vw, 100vw"
                  className={`object-cover transition-transform duration-500 group-hover:scale-105 ${p.imagePosition ?? ""}`}
                  {...(imgId ? imgBind(imgId) : {})}
                />
              </div>
              <div className="p-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-heading text-[16px] font-bold text-text-dark">
                    <MT id={cid(`personas.items.${i}.title`)}>{p.title}</MT>
                  </h3>
                </div>
                <p className="mt-2 text-[14px] leading-relaxed text-text-medium">
                  <MT id={cid(`personas.items.${i}.body`)}>{p.body}</MT>
                </p>
              </div>
            </div>
          );
